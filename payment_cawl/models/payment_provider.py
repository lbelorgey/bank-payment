...
try:
    ...
    _logger.warning(
        "CAWL SDK not available. Please install: pip install onlinepayments-sdk-python3"
    )
...
class PaymentProvider(models.Model):
    ...
    code = fields.Selection(
        selection_add=[("cawl", "CAWL")], ondelete={"cawl": "set default"}
    )
    ...
    def _compute_feature_support_fields(self):
        ...
        cawl_providers.support_refund = (
            "partial"  # CAWL supports partial refunds (like Stripe)
        )
        cawl_providers.support_tokenization = (
            False  # Hosted checkout doesn't support tokenization
        )

    @api.constrains("state")
    def _check_cawl_configuration(self):
        cawl_providers = self.filtered(
            lambda p: p.code == "cawl" and p.state != "disabled"
        )
        for provider in cawl_providers:
            ...
            if not provider.cawl_webhook_secret:
                raise ValidationError(
                    _("Webhook Secret is required for CAWL provider.")
                )
    ...
    def _test_cawl_api_connection(self):
        ...
        try:
            ...
            _logger.info(
                "Configuration values - endpoint: %s, api_key: %s, merchant_id: %s",
                api_endpoint,
                self.cawl_api_key[:8] + "...",
                self.cawl_pspid,
            )
            ...
            try:
                ...
                try:
                    ...
                    _logger.info(
                        "Connection test successful! Checkout ID: %s",
                    ...
                    if (
                        "9007" in str(checkout_error)
                        or "access_to_merchant_not_allowed" in error_str
                    ):
                        return False, _(
                            "Merchant access denied - verify your merchant ID and API key permissions"
                        )
                    elif (
                        "401" in error_str
                        ...
                        return False, _(
                            "Access forbidden - verify API permissions and merchant configuration"
                        )
                    elif "404" in error_str or "not found" in error_str:
                        return False, _(
                            "Merchant ID not found - check your PSPID for the selected environment"
                        )
                ...
                if (
                    ...
                    return False, _(
                        "Authentication failed - check your API credentials and environment setting"
                    )
                elif "404" in error_str or "not found" in error_str:
                    return False, _(
                        "Merchant ID not found - check your PSPID is correct for the selected environment"
                    )
                elif "403" in error_str or "forbidden" in error_str:
                    return False, _(
                        "Access forbidden - check your API permissions and merchant configuration"
                    )
                elif "timeout" in error_str or "connection" in error_str:
                    return False, _(
            ...
            return False, _(
                "Connection test failed: %s\n\nFor debugging, check the server logs for full details."
            ) % str(e)
    ...
    def _validate_webhook_credentials(self):
        ...
        try:
            ...
            response = requests.post(
                url, json=validation_data, headers=headers, timeout=30
            )

            if response.status_code == 200:
                _logger.info(
                    "Webhook credentials validated successfully for provider %s",
                    self.name,
                )
                return True
            else:
                _logger.error(
                    "Webhook credentials validation failed for provider %s: %s",
                    self.name,
                    response.text,
                )
                return False
            ...
            _logger.error(
                "Error validating webhook credentials for provider %s: %s",
                self.name,
                str(e),
            )
            return False
    ...
    def _fetch_cawl_payment_products(self):
        ...
        try:
            ...
            response = (
                client.merchant(self.cawl_pspid).products().get_payment_products(query)
            )
            ...
            for product in response.payment_products:
                ...
                if (
                    hasattr(product, "display_hints")
                    and product.display_hints
                    and hasattr(product.display_hints, "label")
                ):
            ...
            _logger.info(
                "Successfully fetched %d payment products using CAWL SDK", len(products)
            )
            return products
            ...
            if "authentication" in str(e).lower() or "unauthorized" in str(e).lower():
                raise UserError(
                    _("Authentication failed. Please check your API credentials.")
                )
            elif "forbidden" in str(e).lower():
                raise UserError(
                    _(
                        "Access forbidden. Your API credentials don't have permission to fetch payment products."
                    )
                )
    ...
    def _sync_payment_products(self, products_data):
        ...
        for product_data in products_data:
            try:
                ...
                name = None
                if (
                    "displayHints" in product_data
                    and "label" in product_data["displayHints"]
                ):
                ...
                payment_method = (
                    product_data.get("paymentMethod")
                    or product_data.get("method")
                    or "unknown"
                )

                # Check if product already exists
                existing_product = existing_products.filtered(
                ...
                _logger.error(
                    "Error processing product data %s: %s", product_data, str(e)
                )
                continue
        ...
        inactive_products = existing_products.filtered(
            lambda p: p.product_id not in api_product_ids
        )
        ...
        _logger.info(
            "Synced %d payment products for provider %s", len(products_data), self.name
        )
