...
class PaymentTransaction(models.Model):
    ...
    cawl_payment_id = fields.Char(
        string="CAWL Payment ID",
        help="The payment ID returned by CAWL"
    )
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        try:
            ...
            _logger.info("CAWL checkout session created successfully for transaction %s: %s",
                        self.reference, checkout_response.hosted_checkout_id)

        except Exception as e:
            _logger.error("CAWL checkout session creation failed for transaction %s: %s",
    ...
    def _cawl_create_checkout_session(self):
        ...
        try:
            ...
            response = client.merchant(provider.cawl_pspid).hosted_checkout().create_hosted_checkout(checkout_request)

            _logger.info("CAWL checkout session created successfully: %s", response.hosted_checkout_id)
            return response

        except Exception as e:
            _logger.error("CAWL API error during checkout creation for transaction %s: %s",
                         self.reference, str(e))

            # Map CAWL API errors to user-friendly messages
            if "authentication" in str(e).lower() or "unauthorized" in str(e).lower():
                raise UserError(_("Payment service configuration error. Please contact support."))
            elif "network" in str(e).lower() or "connection" in str(e).lower():
                raise UserError(_("Unable to connect to payment service. Please check your internet connection and try again."))
                ...
                raise UserError(_("Invalid payment amount. Please check the order total and try again."))
            else:
    ...
    def _cawl_prepare_checkout_request(self):
        ...
        hosted_checkout_input.show_result_page = False  # Redirect directly back to merchant

        cardPaymentMethodSpecificInputForHostedCheckout = CardPaymentMethodSpecificInputForHostedCheckout()
        cardPaymentMethodSpecificInputForHostedCheckout.group_cards = True  # Group similar cards together
        hosted_checkout_input.card_payment_method_specific_input = cardPaymentMethodSpecificInputForHostedCheckout

        # Add product filtering if configured
        if self.provider_id.cawl_use_product_filter and self.provider_id.cawl_allowed_product_ids:
            allowed_product_ids = [product.product_id for product in self.provider_id.cawl_allowed_product_ids if product.is_active]
            if allowed_product_ids:
                ...
                hosted_checkout_input.payment_product_filters = paymentProductFiltersHostedCheckout
                _logger.info("CAWL hosted checkout configured with %d allowed products", len(allowed_product_ids))
    ...
    def _process_payment_status_update(self, notification_data):
        ...
        _logger.info("Processing payment status update for transaction %s: %s",
                    self.reference, pprint.pformat(notification_data))

        # Extract payment information
        payment_data = notification_data.get('payment', {})
        if not payment_data:
            _logger.warning("No payment data found in notification for transaction %s", self.reference)
            return
        ...
        if not payment_id:
            _logger.warning("No payment ID found in notification for transaction %s", self.reference)
            return
        ...
        if webhook_reference != self.reference:
            _logger.info("Ignoring webhook for different transaction reference: %s (ours: %s)",
                        webhook_reference, self.reference)
            return
        ...
        if payment_id != self.cawl_payment_id:
            if self.cawl_payment_id:
                _logger.info("Updating CAWL payment ID for transaction %s: %s -> %s",
                            self.reference, self.cawl_payment_id, payment_id)
            else:
                _logger.info("Setting CAWL payment ID for transaction %s: %s", self.reference, payment_id)
        ...
        _logger.info("CAWL webhook processed successfully for transaction %s", self.reference)

    def _process_refund_notification(self, notification_data):
        """ Process refund notification from CAWL webhook. """
        _logger.info("Processing refund notification for transaction %s: %s",
                    self.reference, pprint.pformat(notification_data))

        # Extract refund information
        refund_data = notification_data.get('refund', {})
        if not refund_data:
            _logger.warning("No refund data found in notification for transaction %s", self.reference)
            return
        ...
        _logger.info("Refund %s for transaction %s: status=%s, code=%s",
                    refund_id, self.reference, refund_status, refund_status_code)
        ...
        if 'refundOutput' in refund_data:
            ...
            if 'references' in refund_output:
                merchant_reference = refund_output['references'].get('merchantReference')

        if not merchant_reference:
            _logger.warning("No merchant reference found in refund data for transaction %s", self.reference)
            return
        ...
        if not refund_tx:
            _logger.warning("No refund transaction found for merchant reference %s", merchant_reference)
            return
        ...
        if refund_status == 'REFUNDED' and refund_status_code == 8:
            ...
            refund_tx._set_done()
            _logger.info("Refund transaction %s marked as done (REFUNDED)", refund_tx.reference)

            # Trigger post-processing to update order/payment records (like Stripe does)
            try:
                self.env.ref('payment.cron_post_process_payment_tx')._trigger()
                _logger.info("Triggered post-processing for refund transaction %s", refund_tx.reference)
            except Exception as e:
                _logger.warning("Failed to trigger post-processing for refund %s: %s", refund_tx.reference, str(e))

        elif refund_status == 'REFUND_REQUESTED' and refund_status_code == 81:
            # Refund is pending
            refund_tx._set_pending()
            _logger.info("Refund transaction %s marked as pending (REFUND_REQUESTED)", refund_tx.reference)

        elif refund_status in ['REFUSED', 'DECLINED'] or refund_status_code in [83, 84]:
            # Refund was refused or declined
            refund_tx._set_error(f"Refund {refund_status} by CAWL (code: {refund_status_code})")
            _logger.warning("Refund transaction %s marked as error: %s (code: %s)",
                           refund_tx.reference, refund_status, refund_status_code)

        elif refund_status_code == 82:  # Uncertain refund
            # Keep in pending state but log the uncertainty
            refund_tx._set_pending()
            _logger.info("Refund transaction %s marked as pending (UNCERTAIN_REFUND)", refund_tx.reference)

        else:
            _logger.info("Refund transaction %s status unchanged: %s (code: %s)",
                        refund_tx.reference, refund_status, refund_status_code)
    ...
    def _cawl_prepare_customer_data(self):
        ...
        if partner.name:
            ...
            if len(name_parts) >= 2:
                ...
                personal_name.surname = ' '.join(name_parts[1:])  # Join remaining parts as surname
        ...
        if partner.street or partner.city:
            ...
            billing_address.country_code = partner.country_id.code if partner.country_id else 'FR'
    ...
    def _process_notification_data(self, notification_data):
        ...
        if not webhook_type:
            _logger.warning("No webhook type found in notification data for transaction %s", self.reference)
            return
        ...
        if webhook_id:
            ...
            if self.state in ['done', 'error'] and webhook_type in ['payment.created', 'payment.pending_capture']:
                _logger.info("Skipping webhook %s for transaction %s: transaction already in final state %s",
                           webhook_id, self.reference, self.state)
                return
            ...
            if webhook_type in ['payment.pending_capture', 'payment.capture_requested', 'payment.captured']:
        ...
        if 'payment' in notification_data:
            ...
            if 'paymentOutput' in payment_data:
                payment_output = payment_data['paymentOutput']
                if 'references' in payment_output:
                    merchant_reference = payment_output['references'].get('merchantReference')
        ...
        if not merchant_reference or merchant_reference != self.reference:
            _logger.info("Ignoring webhook for transaction %s: merchant reference '%s' doesn't match",
                        self.reference, merchant_reference)
            return
        ...
        if webhook_type in [
            ...
            _logger.info("Processing payment event for transaction %s: %s", self.reference, webhook_type)
            try:
                self._process_payment_status_update(notification_data)
            except Exception as e:
                _logger.error("Error processing payment webhook %s for transaction %s: %s",
                            webhook_type, self.reference, str(e))
        ...
        # Refund events we care about
            ...
            _logger.info("Processing refund event for transaction %s: %s", self.reference, webhook_type)
            try:
                self._process_refund_notification(notification_data)
            except Exception as e:
                _logger.error("Error processing refund webhook %s for transaction %s: %s",
                            webhook_type, self.reference, str(e))
                # Don't re-raise - we want to return 200 to CAWL even if processing fails
            return
        ...
        # Payment link events (ignore for now)
            ...
            _logger.info("Ignoring payment link event for transaction %s: %s", self.reference, webhook_type)
            return

        # Unknown event types - log and ignore
        else:
            _logger.info("Ignoring unknown webhook event for transaction %s: %s", self.reference, webhook_type)
            return

    def _cawl_process_payment_status(self, status, notification_data):
        """ Process CAWL payment status and update transaction state. """
        _logger.info("Processing CAWL status '%s' for transaction %s (current state: %s)",
        ...
        _logger.info("Status transition: %s (level %s) -> %s (level %s)",
                    self.state, current_level, status, new_level)

        # Prevent downgrading transaction state (except for cancellations/rejections)
        if new_level < current_level and new_level >= 0:
            _logger.warning("Ignoring webhook status downgrade: %s -> %s for transaction %s",
                           self.state, status, self.reference)
            return
        ...
        if status in ['CAPTURED', 'PAID']:
            ...
            if self.state != 'done':
                ...
                self._set_done()
                _logger.info("Transaction %s set to done (status: %s)", self.reference, status)
            else:
                _logger.info("Transaction %s already done, ignoring status: %s", self.reference, status)
            ...
            if self.state not in ['done', 'error']:
                ...
                if self.provider_id.capture_manually:
                    ...
                    _logger.info("Transaction %s set to authorized (PENDING_CAPTURE - manual capture enabled)", self.reference)
                else:
                    # For automatic capture, set to pending and try to capture
                    self._set_pending()
                    _logger.info("Transaction %s set to pending (PENDING_CAPTURE - attempting automatic capture)", self.reference)

                    try:
                        self._cawl_capture_payment()
                        _logger.info("Automatic capture successful for payment %s", self.reference)
                    except Exception as e:
                        _logger.error("Automatic capture failed for payment %s: %s", self.reference, str(e))
                        # Keep in pending state - user can manually capture
                        _logger.info("Payment %s remains in pending state - manual capture may be required", self.reference)
            else:
                _logger.info("Transaction %s already in final state %s, ignoring PENDING_CAPTURE",
            ...
            if self.state not in ['done', 'error']:
                self._set_pending()
                _logger.info("Transaction %s set to pending (status: %s)", self.reference, status)
            else:
                _logger.info("Transaction %s already in final state %s, ignoring status: %s",
                           self.reference, self.state, status)

        elif status in ['CANCELLED', 'CANCELLED_BY_CONSUMER']:
            # Payment cancelled - can always cancel
            self._set_canceled()
            _logger.info("Transaction %s cancelled (status: %s)", self.reference, status)
            ...
            _logger.info("Transaction %s set to error (status: %s)", self.reference, status)

        else:
            # Unknown status
            _logger.warning("Unknown CAWL payment status '%s' for transaction %s", status, self.reference)
    ...
    def _get_user_friendly_error_message(self, error_string):
        ...
        if "sdk not available" in error_lower or "import" in error_lower:
            return _("Payment service is temporarily unavailable. Please try again later or contact support.")
            ...
            return _("Unable to connect to payment service. Please check your internet connection and try again.")
        elif "currency" in error_lower and ("invalid" in error_lower or "not supported" in error_lower):
            return _("This currency is not supported for CAWL payments.")
        elif "amount" in error_lower and ("invalid" in error_lower or "minimum" in error_lower):
            return _("Invalid payment amount. Please check the order total and try again.")
        elif "merchant" in error_lower and ("invalid" in error_lower or "not found" in error_lower):
            return _("Payment service configuration error. Please contact support.")
        elif "customer" in error_lower or "contact" in error_lower:
            return _("Customer information is incomplete. Please verify your details and try again.")
        else:
            # Generic fallback for unknown errors
            return _("Unable to create payment session. Please try again or contact support.")
    ...
    def _cawl_capture_payment(self):
        ...
        if self.state != 'pending':
            _logger.warning("Cannot capture payment %s in state %s", self.reference, self.state)
            return
        ...
        try:
            ...
            from onlinepayments.sdk.domain.amount_of_money import AmountOfMoney
            amount_of_money = AmountOfMoney()
            amount_of_money.amount = int(float_round(self.amount * 100, precision_digits=0))  # Convert to cents
            amount_of_money.currency_code = self.currency_id.name
            capture_request.amount_of_money = amount_of_money

            # Capture the payment
            response = client.merchant(provider.cawl_pspid).payments().capture_payment(
                self.cawl_payment_id, capture_request
            )

            _logger.info("CAWL payment capture successful for %s: %s", self.reference, response.id)
            ...
            _logger.error("CAWL payment capture failed for %s: %s", self.reference, str(e))
            ...
            if "authentication" in str(e).lower() or "unauthorized" in str(e).lower():
                ...
                raise UserError(_("Invalid capture amount. Please check the amount and try again."))
            else:
    ...
    def _send_refund_request(self, amount_to_refund=None):
        ...
        try:
            ...
            refund_response = self._cawl_create_refund(amount_to_refund, refund_tx.reference)
            ...
            _logger.info(
                ...
                self.reference, refund_response.id
            )
    ...
    def _cawl_check_refund_eligibility(self):
        ...
        _logger.info("Transaction %s is in 'done' state, allowing refunds", self.reference)
    ...
    def _cawl_create_refund(self, amount_to_refund, refund_reference):
        ...
        refund_request = self._cawl_prepare_refund_request(amount_to_refund, refund_reference)

        try:
            # Create refund through CAWL API
            response = client.merchant(provider.cawl_pspid).payments().refund_payment(
                self.cawl_payment_id, refund_request
            )
            ...
            _logger.error("CAWL API error during refund creation for transaction %s: %s",
                         self.reference, str(e))

            # Log additional details for debugging
            _logger.error("Transaction details - ID: %s, State: %s, Operation: %s, CAWL Payment ID: %s",
                         self.id, self.state, self.operation, self.cawl_payment_id)

            # Check if we can get payment details to understand the current state
            try:
                payment_details = client.merchant(provider.cawl_pspid).payments().get_payment(self.cawl_payment_id)
                _logger.error("CAWL Payment details - Status: %s, Status Code: %s",
                             getattr(payment_details, 'status', 'Unknown'),
                             getattr(payment_details, 'status_code', 'Unknown'))
            except Exception as detail_error:
                _logger.error("Could not retrieve payment details: %s", str(detail_error))
            ...
            if "authentication" in str(e).lower() or "unauthorized" in str(e).lower():
                ...
                raise UserError(_("Invalid refund amount. Please check the amount and try again."))
            elif "ACTION_NOT_ALLOWED_ON_TRANSACTION" in str(e):
                raise UserError(_("Refund not allowed on this transaction. The payment may not be settled yet or refunds may not be supported for this payment method. Please contact support."))
            else:
                raise UserError(_("Payment gateway error: %s") % str(e))
