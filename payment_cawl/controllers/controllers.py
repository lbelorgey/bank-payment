...
class CawlController(http.Controller):
    ...
    @http.route(
        '/payment/cawl/return/<int:tx_id>', type='http', auth='public', 
    ...
    def cawl_return_from_checkout(self, tx_id, **data):
        ...
        _logger.info("Handling return from CAWL for transaction %s with data:\n%s", tx_id, pprint.pformat(data))
    ...
    @http.route(
        '/payment/cawl/webhook', type='http', auth='public', 
    ...
    def cawl_webhook(self, **data):
        ...
        _logger.info("Content-Type: %s", request.httprequest.headers.get('Content-Type', 'Not set'))
        _logger.info("User-Agent: %s", request.httprequest.headers.get('User-Agent', 'Not set'))
    ...
    def _handle_return_data(self, tx_sudo, data):
        ...
        _logger.info("Processing return data for transaction %s: %s", tx_sudo.reference, pprint.pformat(data))
        ...
        if not checkout_id:
            _logger.warning("No checkout ID in return data for transaction %s", tx_sudo.reference)
        ...
        _logger.info("Successfully processed return data for transaction %s - state unchanged, webhook will handle status", tx_sudo.reference)
    ...
    def _handle_webhook_notification(self, webhook_data, signature, data):
        ...
        try:
            ...
            _logger.error("Invalid JSON in CAWL webhook data: %s, Error: %s", webhook_data, str(e))
            raise ValidationError("Invalid webhook data format")

        _logger.info("Processing CAWL webhook notification: %s", pprint.pformat(notification_data))

        # Find the transaction
        tx_sudo = self._get_tx_from_webhook_data(notification_data)
        if not tx_sudo:
            _logger.warning("Could not find transaction for webhook data: %s", pprint.pformat(notification_data))
            return

        # Verify webhook signature - webhook secret is MANDATORY for security
        if not tx_sudo.provider_id.cawl_webhook_secret:
            _logger.error("Webhook secret not configured for provider %s - REJECTING webhook for security", tx_sudo.provider_id.name)
            raise Forbidden("Webhook secret not configured - webhook rejected for security")
        
        if not signature:
            _logger.error("No signature provided in webhook request - REJECTING webhook for security")
            raise Forbidden("No webhook signature provided - webhook rejected for security")
        
        # Validate webhook signature
        if not self._verify_webhook_signature(tx_sudo.provider_id, webhook_data, signature):
            _logger.error("Invalid webhook signature for transaction %s - REJECTING webhook", tx_sudo.reference)
    ...
    def _get_tx_from_webhook_data(self, notification_data):
        ...
        if checkout_id:
            tx_sudo = request.env['payment.transaction'].sudo().search([
                ('cawl_checkout_id', '=', checkout_id)
            ], limit=1)

        # Try to find by payment ID
        if not tx_sudo and payment_id:
            tx_sudo = request.env['payment.transaction'].sudo().search([
                ('cawl_payment_id', '=', payment_id)
            ], limit=1)
    ...
    def _verify_webhook_signature(self, provider, webhook_data, signature):
        ...
        try:
            ...
            hmac_obj = hmac.new(
                ...
                webhook_data.encode('utf-8') if isinstance(webhook_data, str) else webhook_data,
                hashlib.sha256
            )
            ...
            _logger.info("Signature verification - Expected: %s, Received: %s", expected_signature, signature)
    ...
    @http.route('/payment/cawl/simulate_return', type='http', auth='user', methods=['GET'])
    ...
    @http.route('/payment/cawl/simulate_webhook', type='http', auth='user', methods=['POST'])
    ...
    @http.route('/payment/cawl/test_webhook', type='http', auth='public', methods=['GET'])
    def cawl_test_webhook(self):
        """ Test endpoint to verify webhook URL is accessible. """
        return 'CAWL Webhook endpoint is accessible!'

    @http.route('/payment/cawl/update_status/<int:tx_id>', type='http', auth='user', methods=['GET'])
